...
export default function Dashboard({ portfolio, positions, signals, trades, logs, equityHistory, loading }: DashboardProps) {
  ...
  // Use bot-reported snapshot as source of truth for account state
  ...
  // Cash = equity minus value locked in open positions
  ...
  // Trade volume stats
  const totalBuyVolume = trades.filter((t: any) => t.action === "BUY").reduce((s: number, t: any) => s + (t.value || (t.qty ?? 0) * (t.price ?? 0)), 0);
  const totalSellVolume = trades.filter((t: any) => t.action === "SELL").reduce((s: number, t: any) => s + (t.value || (t.qty ?? 0) * (t.price ?? 0)), 0);
